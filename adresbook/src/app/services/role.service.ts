...
export class RoleService {
  ...
  patchRole(id: number, xApiKey: string, adresBody?: RoleBody): Observable<HttpResponse<Role>> {
    const headers: HttpHeaders = new HttpHeaders({
      'x-api-key': xApiKey
    });
    ...
    if (this.api != undefined) {
      return this.api.patchRole(id, xApiKey, adresBody, 'response', false, options);
    } else {
      throw new Error("OpenadresService api not yet defined");
    }
  }
